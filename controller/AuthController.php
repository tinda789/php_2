...
if ($action === 'register' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (User::register($conn, $username, $password, $email, $confirm_password, $first_name, $last_name, $phone, $error)) {
        ...
        $message = "Chào $first_name $last_name,<br>Cảm ơn bạn đã đăng ký tài khoản tại shop của chúng tôi!<br>Bạn có thể đăng nhập ngay bây giờ.";
        ...
        try {
            ...
            $mail->Body    = $message;
            $mail->send();
            // Không cần thông báo, chỉ gửi lặng lẽ
        } catch (Exception $e) {
...
if ($action === 'send_reset' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($user = $result->fetch_assoc()) {
        ...
        $message = "Nhấn vào link sau để đặt lại mật khẩu: $link";
        ...
        try {
            ...
            $mail->Body    = $message;
            $mail->send();
            echo '<div class="container" style="max-width:400px;margin:40px auto;"><div class="alert alert-success">Đã gửi link khôi phục về email. Vui lòng kiểm tra hộp thư!</div></div>';
        } catch (Exception $e) {
...
if ($action === 'save_new_password' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_SESSION['reset_otp_verified']) || !$_SESSION['reset_otp_verified']) {
        die('Bạn chưa xác thực OTP!');
    }
    $token = $_POST['token'] ?? '';
    $new_password = $_POST['new_password'] ?? '';
    $email = $_SESSION['reset_otp_email'] ?? '';
    if (!$new_password || !$email) die('Thiếu thông tin');
    $hash = password_hash($new_password, PASSWORD_DEFAULT);
    $stmt = $conn->prepare("UPDATE users SET password_hash=?, reset_token=NULL, reset_expires=NULL WHERE email=?");
    $stmt->bind_param("ss", $hash, $email);
    $stmt->execute();
    unset($_SESSION['reset_otp'], $_SESSION['reset_otp_expire'], $_SESSION['reset_otp_email'], $_SESSION['reset_otp_verified']);
    echo '<div class="container" style="max-width:400px;margin:40px auto;"><div class="alert alert-success">Đặt lại mật khẩu thành công! Bạn có thể <a href=\'index.php?controller=auth&action=login\'>đăng nhập</a> ngay.</div></div>';
    exit;
}
